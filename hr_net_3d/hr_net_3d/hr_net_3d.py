            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    y = y + F.interpolate(
                        ...
                        size=[x[i].shape[2], x[i].shape[3], x[i].shape[4]],
                        mode='trilinear')
                else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, out_chs, architecture: dict = None):
        # TODO: pass stages as list instead of dict with keys stage2, stage3, ...
        ...
        self.stage2_cfg = architecture['stage2']
        ...
        self.stage3_cfg = architecture['stage3']
        ...
        self.stage4_cfg = architecture['stage4']
        ...
        self.head = nn.Sequential(
            nn.Conv3d(
                ...
                kernel_size=1,
                stride=1,
                padding=1 if architecture['final_conv_kernel'] == 3 else 0),
            BatchNorm3d(final_inp_channels, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                in_channels=final_inp_channels,
                out_channels=out_chs,
                kernel_size=architecture['final_conv_kernel'],
                stride=1,
                padding=1 if architecture['final_conv_kernel'] == 3 else 0)
        )
    ...
    def forward(self, x):
        ...
        # Head Part
        height, width, depth = x[0].size(2), x[0].size(3), x[0].size(4)
        x1 = F.interpolate(x[1], size=(height, width, depth), mode='trilinear', align_corners=False)
        x2 = F.interpolate(x[2], size=(height, width, depth), mode='trilinear', align_corners=False)
        x3 = F.interpolate(x[3], size=(height, width, depth), mode='trilinear', align_corners=False)
        x_feat = torch.cat([x[0], x1, x2, x3], 1)
